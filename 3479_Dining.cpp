...
*
*/
/*如果顺序为:源点0——食物f——牛n——饮料d——汇点t
那可能出现牛被复用的情况(只是通过)
故修改顺序为：源点0——食物f——牛n——牛n——饮料d——汇点t
*/
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
...
int main() {
	int i, j, k, s, t, n, f, d, food_num, drink_num, tmp_f, tmp_d;
	...
	for (i = 1; i <= f; i++)
		add_edge(s, i, 1, 0);
	for (i = 1; i <= n; i++)
		add_edge(i + f, i + f + n, 1, 0);
	for (i = 1; i <= n; i++) {
		cin >> food_num >> drink_num;
		for (j = 1; j <= food_num; j++) {
			cin >> tmp_f;
			add_edge(tmp_f, i + f, 1, 0);
		}
		for (j = 1; j <= drink_num; j++) {
			cin >> tmp_d;
			add_edge(i + n + f, tmp_d + f + 2 * n, 1, 0);
		}
	}
	for (i = 1; i <= d; i++)
		add_edge(i + 2 * n + f, t, 1, 0);
